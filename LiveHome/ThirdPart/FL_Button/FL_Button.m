...
- (instancetype)initWithAlignmentStatus:(FLAlignmentStatus)status{
    ...
    fl_button.fl_buttonTopRadio = 0.8;
    return fl_button;
...
- (void)alignmentCenter{
    ...
    //    设置图片的坐标
    CGFloat imageX = CGRectGetMaxX(self.titleLabel.frame) + 3;
    CGFloat imageY = (fl_btnHeight - fl_imageHeight) * 0.5;
    //    设置图片的frame
    self.imageView.frame = CGRectMake(imageX, imageY, fl_imageWidth, fl_imageHeight);
}

#pragma mark - 正常左图 右边文字 居中对齐
- (void)alignmentNorCenter{
    
    CGSize buttonTitleLabelSize = [self sizeWithText:self.buttonName font:[UIFont systemFontOfSize:self.fontSiz] maxW:150 maxH:20];
    CGFloat  titleWidth = buttonTitleLabelSize.width;
    CGFloat titleHeight = buttonTitleLabelSize.height;
    
    //    CGFloat  titleWidth = fl_labelWidth;
    //    CGFloat titleHeight = fl_labelHeight;

    //    设置文本的坐标
    CGFloat imageX= (fl_btnWidth - titleWidth -fl_imageWidth - fl_padding) * 0.5;
    CGFloat imageY= (fl_btnHeight - titleHeight) * 0.5;
    ...
    CGFloat labelY = (fl_btnHeight - titleHeight) * 0.5;
    
    self.titleLabel.frame = CGRectMake(labelX, labelY, titleWidth, titleHeight);
...
- (void)alignmentTop{
    ...
    self.imageView.frame = CGRectMake(imageX, fl_btnHeight * 0.5 - fl_imageHeight * _fl_buttonTopRadio, fl_imageWidth, fl_imageHeight);
    self.titleLabel.frame = CGRectMake((self.center.x - frame.size.width) * 0.5, fl_btnHeight * 0.5 + fl_labelHeight * _fl_buttonTopRadio, fl_labelWidth, fl_labelHeight);
